define(['knockout', 'messages', 'utilities'], function (ko, messages, utilities) {
	...
	return function vmOctal() {
		...
		messages.subscribe('clock.update', function (time) {
			_this.visible(true);
			_this.hour(utilities.leadingZero(time.hour.toString(16), 2));
			_this.minute(utilities.leadingZero(time.minute.toString(16), 2));
			_this.second(utilities.leadingZero(time.second.toString(16), 2));			
		});
